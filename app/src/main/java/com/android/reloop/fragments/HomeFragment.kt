 ...
 */
class HomeFragment : BaseFragment(), View.OnClickListener, OnNetworkResponse, AlertDialogCallback ,HomeImagesPagerAdapter.ItemClickListener{
    companion object {
        private var isOpenDropOff: Boolean = false
        private var dropOffId: String = ""

        fun newInstance(openDropOff: Boolean,dropOffIdStr: String): HomeFragment {
    ...
    var textOrganization: TextView ?= null
...
//        val view: View? = inflater.inflate(R.layout.fragment_home, container, false) //OLD design
        ...
        val view: View? = inflater.inflate(R.layout.fragment_shop_main_screen, container, false)//NEW design
    ...
    private fun setInfiniteViewpager(list: ArrayList<String>?) {
        ...
        mPagerAdapter = InfinitePagerAdapter(myCustomPagerAdapter)
    ...
    private fun initViews(view: View?) {
        ...
        textOrganization = view?.findViewById(R.id.textOrganization)
    ...
    private fun setTextsOnTop(){
        if (MainApplication.userType() == Constants.UserType.household) {
            ...
            if(User.retrieveUser()!=null && User.retrieveUser()!!.organization!=null) {
                if (User.retrieveUser()!!.organization!!.is_master == 1) {
                    textOrganization!!.text = "Numbers on this page include: Master Organization, Connected Branches and Connected Households"
                }else{
                    textOrganization!!.text = "Numbers on this page include: Organization and Connected Households"
                }
            }
        }
    }
    ...
    private fun populateRecyclerViewData(
        ...
        try{
        dataList.clear()
        dataList.add(ModelHomeCategories("Saving", R.drawable.icon_tree_new, environmentalStats?.trees_saved!!, "", "Trees", EnvironmentalStatsDescriptionsIDs.TREES_SAVED_ID, ""))
        dataList.add(ModelHomeCategories("Saving", R.drawable.icon_barrel_new, environmentalStats.oil_saved, "L", "of Oil", EnvironmentalStatsDescriptionsIDs.OIL_SAVED_ID, ""))
            dataList.add(ModelHomeCategories("Saving",
        ...
        when (tag) {
            ...
            RequestCodes.API.DASHBOARD -> {
                try {
                    ...
                    val dashboard = Gson().fromJson(Utils.jsonConverterObject(baseResponse?.data as? LinkedTreeMap<*, *>), Dashboard::class.java)
                    Log.e("TAG","===response2===" + dashboard)

                    Log.e("TAG","===response home===" + dashboard.userProfile?.first_name)
                    Log.e("TAG","===response home===" + dashboard.userProfile?.last_name)
                    Log.e("TAG","===response home===" + dashboard.userProfile?.addresses)
                    ...
                    userModel?.addresses = dashboard.userProfile?.addresses
                    userModel?.reward_points = dashboard.rewardPoints

                   //NEW ADDED
                    userModel?.driver_photo_visibility = dashboard.userProfile?.driver_photo_visibility
                    userModel?.supervisor_photo_visibility = dashboard.userProfile?.supervisor_photo_visibility
//to save address to use in report screen
                    val sharedPreferences: SharedPreferences = requireContext().getSharedPreferences("shared preferences", MODE_PRIVATE)
                    val editor = sharedPreferences.edit()
                    val gson = Gson()
                    Log.e("TAG","===response address saved===" + dashboard.userProfile!!.addresses)
                    val json = gson.toJson(dashboard.userProfile?.addresses)
                    Log.e("TAG","===response address saved json===" +json)
                    ...
                    populateData(dashboard.environmentalStats, dashboard.environmentalStatsDescriptions, dashboard.totalRecycledKg, dashboard.rewardPoints!!)
                   ...
                   if (!settings.campaigns_visibility.isNullOrEmpty()) {
                        ...
                        if(settings.campaigns_visibility.get(0).value.equals("1"))
                        {
                            ...
                            //callListingApi()  // stop api call and get data from dashboard api
                            campaignList = dashboard.campaigns!!
...
//                        || User.retrieveUser()?.birth_date.isNullOrEmpty() //Earlier mandatory, now optional
                    ...
                    //Show texts at bottom of home screen
                   if (!settings.show_text_on_bottom_of_home_screen.isNullOrEmpty()) {
                        if(settings.show_text_on_bottom_of_home_screen.get(0).value.equals("1")) {
                            val tinyDB: TinyDB?
                            tinyDB = TinyDB(MainApplication.applicationContext())
                            tinyDB.putString("home_text_visibility", "VISIBLE")
                            ...
                            val tinyDB: TinyDB?
                            tinyDB = TinyDB(MainApplication.applicationContext())
                            tinyDB.putString("home_text_visibility", "IN_VISIBLE")
                            textOrganization!!.visibility = View.GONE
                   ...
                   if (!settings.coffee_capsules_household_visibility.isNullOrEmpty() &&
                    ...
                    if (settings.reports_download_button_for_household.isNotEmpty() &&
                        settings.reports_download_button_for_organization.isNotEmpty()) {
                    ...
                    //driver_photo_visibility_to_households visibility
                    if (!settings.driver_photo_visibility_to_households.isNullOrEmpty()) {
                        if(settings.driver_photo_visibility_to_households.get(0).value.equals("1")) {
                            val tinyDB: TinyDB?
                            tinyDB = TinyDB(MainApplication.applicationContext())
                            tinyDB.putString("driver_house_vis", "VISIBLE")

                        } else{
                            val tinyDB: TinyDB?
                            tinyDB = TinyDB(MainApplication.applicationContext())
                            tinyDB.putString("driver_house_vis", "IN_VISIBLE")
                        }
                    } else{
                        val tinyDB: TinyDB?
                        tinyDB = TinyDB(MainApplication.applicationContext())
                    ...
                    //driver_photo_visibility_to_organizations visibility
                    if (!settings.driver_photo_visibility_to_organizations.isNullOrEmpty()) {
                        if(settings.driver_photo_visibility_to_organizations.get(0).value.equals("1")) {
                            val tinyDB: TinyDB?
                            tinyDB = TinyDB(MainApplication.applicationContext())
                            tinyDB.putString("driver_org_vis", "VISIBLE")

                        } else{
                            val tinyDB: TinyDB?
                            tinyDB = TinyDB(MainApplication.applicationContext())
                            tinyDB.putString("driver_org_vis", "IN_VISIBLE")
                        }
                    } else{
                        val tinyDB: TinyDB?
                        tinyDB = TinyDB(MainApplication.applicationContext())
                    ...
                    //supervisor_photo_visibility_to_households visibility
                    if (!settings.supervisor_photo_visibility_to_households.isNullOrEmpty()) {
                        if(settings.supervisor_photo_visibility_to_households.get(0).value.equals("1"))
                        {
                            val tinyDB: TinyDB?
                            tinyDB = TinyDB(MainApplication.applicationContext())
                            tinyDB.putString("sup_house_vis", "VISIBLE")

                        } else{
                            val tinyDB: TinyDB?
                            tinyDB = TinyDB(MainApplication.applicationContext())
                            tinyDB.putString("sup_house_vis", "IN_VISIBLE")
                        }
                    }
                    else{
                        val tinyDB: TinyDB?
                    ...
                    //supervisor_photo_visibility_to_organizations visibility
                    if (!settings.supervisor_photo_visibility_to_organizations.isNullOrEmpty()) {
                        if(settings.supervisor_photo_visibility_to_organizations.get(0).value.equals("1")) {
                            val tinyDB: TinyDB?
                            tinyDB = TinyDB(MainApplication.applicationContext())
                            tinyDB.putString("sup_org_vis", "VISIBLE")

                        } else{
                            val tinyDB: TinyDB?
                            tinyDB = TinyDB(MainApplication.applicationContext())
                            tinyDB.putString("sup_org_vis", "IN_VISIBLE")
                        }
                    } else{
                        val tinyDB: TinyDB?
                        tinyDB = TinyDB(MainApplication.applicationContext())
                    ...
                    //households_collection_photo_visibility visibility
                    if (!settings.households_collection_photo_visibility.isNullOrEmpty()) {
                        if(settings.households_collection_photo_visibility.get(0).value.equals("1")) {
                            val tinyDB: TinyDB?
                            tinyDB = TinyDB(MainApplication.applicationContext())
                            tinyDB.putString("house_col_vis", "VISIBLE")

                        } else{
                            val tinyDB: TinyDB?
                            tinyDB = TinyDB(MainApplication.applicationContext())
                            tinyDB.putString("house_col_vis", "IN_VISIBLE")
                        }
                    } else{
                        val tinyDB: TinyDB?
                        tinyDB = TinyDB(MainApplication.applicationContext())
                    ...
                    //organizations_collection_photo_visibility visibility
                    if (!settings.organizations_collection_photo_visibility.isNullOrEmpty()) {
                        if(settings.organizations_collection_photo_visibility.get(0).value.equals("1")) {
                            val tinyDB: TinyDB?
                            tinyDB = TinyDB(MainApplication.applicationContext())
                            tinyDB.putString("org_col_vis", "VISIBLE")

                        } else{
                            val tinyDB: TinyDB?
                            tinyDB = TinyDB(MainApplication.applicationContext())
                            tinyDB.putString("org_col_vis", "IN_VISIBLE")
                        }
                    } else{
                        val tinyDB: TinyDB?
                        tinyDB = TinyDB(MainApplication.applicationContext())
                    ...
                    //STORE Household Discount amount
                    if (dashboard.hhDiscount != null) {
                        val tinyDB: TinyDB?
                        tinyDB = TinyDB(MainApplication.applicationContext())
                        tinyDB.putString("hhDiscount_ID", ""+ dashboard.hhDiscount!!.id)
                        tinyDB.putString("hhDiscount_PR", ""+ dashboard.hhDiscount!!.discount_validity)
                        tinyDB.putString("hhDiscount_CODE", ""+ dashboard.hhDiscount!!.discount_code)

                    } else{
                        val tinyDB: TinyDB?
                        tinyDB = TinyDB(MainApplication.applicationContext())
                        tinyDB.putString("hhDiscount_ID", "NULL")
                        tinyDB.putString("hhDiscount_PR", "NULL")
                        tinyDB.putString("hhDiscount_CODE", "NULL")
                    }
...
//                        || User.retrieveUser()?.birth_date.isNullOrEmpty() //Earlier mandatory, now optional
                    ...
                    if (newlist.get(0).trips == 0) {
                        ...
                        Log.e(SubscriptionFragment.TAG, "===Event Home Page :newlist else: ${Gson().toJson(User.retrieveUser()?.addresses)}")
                        if(User.retrieveUser()?.addresses?.isNotEmpty()!! && User.retrieveUser()?.addresses!!.size > 0) {
                            ...
                            if(default == 1)
                            {
                                val lat = User.retrieveUser()?.addresses?.get(0)?.latitude
                                val lon = User.retrieveUser()?.addresses?.get(0)?.longitude
                                Log.e(SubscriptionFragment.TAG, "===lat /lon :" + lat + ":" + lon)
                                if ((lat == 0.0 || lon == 0.0)) {
                                    Log.e(SubscriptionFragment.TAG, "===if if:" + lat + ":" + lon)
                                        ...
                                        || User.retrieveUser()?.addresses?.get(0)?.building_name.isNullOrEmpty())
                                    {
                                        ...
                                        Notify.hyperlinkAlert(
                                            activity,
                                            getString(R.string.select_pin_location),
                                            getString(R.string.update_profile_heading),
                                            this,
                                            2
                                        )
                                ...
                                Notify.hyperlinkAlert(
                                    activity,
                                    getString(R.string.set_default_location),
                                    getString(R.string.update_profile_heading),
                                    this,
                                    2
                                )
                                ...
                                if ((lat == 0.0 || lon == 0.0)) {
                                    ...
                                    Notify.hyperlinkAlert(
                                        activity, getString(R.string.update_profile_msg),
                                        getString(R.string.update_profile_heading),
                                        this,
                                        2)
                                    recycle?.isClickable = true
                    ...
                    if(User.retrieveUser()?.addresses?.isNotEmpty()!! && User.retrieveUser()?.addresses!!.size > 0) {
                        ...
                        if(default == 1)
                        {
                            Log.e(SubscriptionFragment.TAG, "===else :")
                            val lat = User.retrieveUser()?.addresses?.get(0)?.latitude
                            val lon = User.retrieveUser()?.addresses?.get(0)?.longitude
                            Log.e(SubscriptionFragment.TAG, "===lat /lon :" + lat + ":" + lon)
                            if ((lat == 0.0 || lon == 0.0)) {
                                    ...
                                    || User.retrieveUser()?.addresses?.get(0)?.building_name.isNullOrEmpty())
                                {
                                    ...
                                    Notify.hyperlinkAlert(
                                        activity, getString(R.string.select_pin_location),
                                        getString(R.string.update_profile_heading),
                                        this,
                                        2
                                    )
                                ...
                                Log.e(
                                    ...
                                    "===Event Home Page :newlist : ${Gson().toJson(newlist)}"
                                )
                                if (newlist.get(0).trips == 0) {
                                    Notify.hyperlinkAlert(
                                        activity,
                            ...
                            Notify.hyperlinkAlert(
                                activity, getString(R.string.set_default_location),
                                getString(R.string.update_profile_heading), this, 2)
                            recycle?.isClickable = true
                        }
                        else {
                            Log.e(SubscriptionFragment.TAG, "===else :")
                ...
                if (User.retrieveUser()?.organization?.name.isNullOrEmpty()
                    ...
                    if (newlist.get(0).trips == 0) {
                        ...
                        if(User.retrieveUser()?.addresses?.isNotEmpty()!! && User.retrieveUser()?.addresses!!.size > 0) {
                            ...
                            {
                                Notify.hyperlinkAlert(activity, getString(R.string.set_default_location), getString(R.string.update_profile_heading), this, 2)
                                recycle?.isClickable = true
                            }
                            else {
                                val lat = User.retrieveUser()?.addresses?.get(0)?.latitude
